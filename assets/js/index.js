...
$(document).ready(function () {
  ...
  a = new App(baseUrl, formattedDataset); 

  // --- Stat lines color

  $(".stat-shape[data-stat-id]").each(function (i, e) {
    var statId = this.dataset.statId;
    $(this).find("svg").attr("vector-effect", "non-scaling-stroke");
    $(this).find("svg path[id^='line-']")
      .addClass("city-stat-path")
      .addClass("stroke-c");
    var n = Math.floor(Math.random() * 20);
    for (var i = 0; i < n; i++) {
      var line = $(this).find("svg #line-"+ i);
      line
        .removeClass("stroke-c")
        .addClass("stroke-a");
    }
  });
...
function Timeline (x, y1, y2) {
  ...
  //   var date = state.storiesMap[slug].date;
  //   var dateString = date.format("D MMM") +"\n"+ date.format("YYYY");
  //   this.date.text(dateString).y(point.cy() - 15);
  //   if (this.timeout) {
  ...
  //     that.date.text(dateString).y(point.cy() - 15);
  //   }, animms);
  // }
  this.mouseover = function (slug) {
    var point = $(".timeline g.point[data-slug='"+ slug +"']")[0].instance;
